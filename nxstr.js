...
const nxstr = (() => {
  ...
  return {
    ...
    sha256: async (data) => {
      ...
      return nxstr.b2h(new Uint8Array(hb));
    },
    ...
    getSecretKey: () => {
      if (confirm("Do you want to export your private key?")) {
        return sk;
      }
      return null;
    },
    ...
    sign: async (event,csk) => {
      ...
      event.id = await nxstr.sha256(data);
      event.sig = await nxstr.schnorr.sign(event.id,csk||sk);

      return event;
    },

    encrypt: async (text,cpk,csk) => {
